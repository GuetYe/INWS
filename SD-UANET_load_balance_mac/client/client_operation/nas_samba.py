...
class Samba:
    ...
    # 添加删除文件的操作
    def delete_file(self, remote_file_path, share_folder=setting.DEFAULT_SHARE_FOLDER_NAME):
        """
        description: 删除远程samba服务器上的指定文件
        param: remote_file_path 远程文件路径（例如：'/test1_part/test1_1.exe'）
        param: share_folder 共享文件夹名称，默认为 setting.DEFAULT_SHARE_FOLDER_NAME
        """
        if self.connect_state:
            try:
                print(f"Attempting to delete file: {remote_file_path} from share: {share_folder}")
                self.smb_client.deleteFiles(share_folder, remote_file_path)
                print(f"Successfully deleted {remote_file_path} from {self.host}")
            except Exception as e:
                print(f"Error deleting {remote_file_path} from {self.host}: {e}")
                raise
        else:
            print("Samba connection is not established. Cannot delete file.")
            return False
