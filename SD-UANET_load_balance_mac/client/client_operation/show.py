...
from PyQt5.QtCore import QTimer, QDateTime, Qt, QRectF, QThread, pyqtSignal 
    ...
    def load_files_from_db(self):
        ...
        try:
            ...
            for result in results:
                file_name = result[0]
                file_path = result[1]  # 获取完整文件路径

                DataManager().add_file(file_name, file_path)
...
class PullThread(QThread):
    ...
    def run(self):
        try:
            save.pull_files(self.file_path)
            self.finished.emit(True, "文件拉取成功！")
        except Exception as e:
            self.finished.emit(False, f"拉取文件时出错：{str(e)}")
def delete_file_record(file_path):
    """
    根据文件路径删除 file_info 数据库中对应的记录，
    同时删除 part_file_info 数据库中对应的分块记录。
    """
    ...
    user_db = 'root'
    password_db = 'guet'
    database_file_info = 'file_info'
    database_part_info = 'part_file_info'

    # 1. 查询 file_info 表，获取该文件的 symbol
    symbol = None
    try:
        connection = pymysql.connect(host=host_db, user=user_db, password=password_db, database=database_file_info)
        cursor = connection.cursor()
        query = "SELECT symbol FROM file_infomation WHERE file_name=%s"
        cursor.execute(query, (file_name,))
        result = cursor.fetchone()
        if result:
            symbol = result[0]
            print(f"Found symbol {symbol} for file {file_name}")
        else:
            print(f"No record found in file_info for {file_name}")
        cursor.close()
        connection.close()
    except Exception as e:
        print("Error querying file_info database:", e)
        return

    # 2. 如果获取到 symbol，则删除 part_file_info 表中对应记录
    if symbol:
        try:
            connection = pymysql.connect(host=host_db, user=user_db, password=password_db, database=database_part_info)
            cursor = connection.cursor()
            delete_query = "DELETE FROM part_file_infomation WHERE origin_symbol=%s"
            cursor.execute(delete_query, (symbol,))
            connection.commit()
            print(f"Part file info records with origin_symbol={symbol} deleted from part_file_info database.")
            cursor.close()
            connection.close()
        except Exception as e:
            print("Error deleting part file info from part_file_info database:", e)

    # 3. 删除 file_info 表中的记录
    try:
        connection = pymysql.connect(host=host_db, user=user_db, password=password_db, database=database_file_info)
        cursor = connection.cursor()
        delete_query = "DELETE FROM file_infomation WHERE file_name=%s"
        cursor.execute(delete_query, (file_name,))
        connection.commit()
        print(f"File info record for {file_name} deleted from file_info database.")
        cursor.close()
        connection.close()
    except Exception as e:
        print("Error deleting file info from file_info database:", e)
...
class DeleteThread(QThread):
    ...
    def run(self):
        try:
            save.delet_files(self.file_path)
            # 删除数据库中的记录
            delete_file_record(self.file_path)
            self.finished.emit(True, "文件删除成功！")
        except Exception as e:
            self.finished.emit(False, f"删除文件时出错：{str(e)}")
...
class ThirdWindow(QMainWindow):
    def __init__(self):
        ...
        self.delete_thread = None  # 新增删除线程属性
        self.current_delete_file = None  # 记录当前正在删除的文件（文件名）
        # 绑定按钮点击事件
    ...
    def delete_file(self):
        ...
        if selected_item:
            ...
            if reply == QMessageBox.Yes:
                self.current_delete_file = file_info  # 记录当前待删除的文件
                ...
                # 创建并启动删除线程
                self.delete_thread = DeleteThread(file_path)
                self.delete_thread.finished.connect(self.handle_delete_result)
    ...
    def handle_delete_result(self, success, message):
        ...
        if success:
            # 更新 DataManager 中的文件列表
            try:
                DataManager().delete_file(self.current_delete_file)
                # DataManager().files.remove(self.current_delete_file)
            except Exception as e:
                print(f"更新 DataManager 失败：{e}")
            self.refresh_file_list()  # 刷新文件列表显示
    ...
    def download_file(self):
        ...
        if not selected_item:
            QMessageBox.warning(self, "下载", "请先选择一个文件！")
            return
        file_info = selected_item.text()
        # 从 DataManager 中获取完整路径
        files = DataManager().get_files()
        file_path = files.get(file_info)
        if not file_path:
            QMessageBox.warning(self, "下载", f"无法找到文件 {file_info} 的完整路径！")
            return
        print("★★★★★★★★★★★★★★★★★★★★★★★★★★★",file_info)
        ...
        # 创建并启动拉取线程
        self.upload_thread = PullThread(file_path)
