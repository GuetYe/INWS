...
if __name__ == '__main__':
    ...
    for number in range(setting.experiment_number):
        ...
        file_path = 'E:/ubuntu18.04镜像/ubuntu-18.04.6-desktop-amd64.iso'
        ...
        if mode == 1:
            ...
            for host_ip, local_file_path in samba_transfer_info.items():
                part_file_name = file_utils.file_name(local_file_path)  # 截取文件名
                remote_file_path = save_remote_path_complete + part_file_name  # 远程文件路径
                insert_file_storage_info(file_name=file_name_complete, host_ip=host_ip,
                                         remote_path=remote_file_path)  # 存储到数据库
