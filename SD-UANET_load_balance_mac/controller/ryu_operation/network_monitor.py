...
LastEditTime: 2024-11-25 17:31:31
...
class Networkmonitor(app_manager.RyuApp):
        ...
        for ofport in body:  
            if ofport.port_no != ofproto_v1_3.OFPP_LOCAL:  # 0xfffffffe  4294967294,本地回环端口
                ...
                try:
                    ...
                    logger.info("ofport.curr_speed:%d  ofport.max_speed:%d", ofport.curr_speed, ofport.max_speed)
                # 端口流速有时在properties中
                except:  
                    ...
                    logger.info("properties[0].curr_speed:%d  properties[0].max_speed:%d", ofport.properties[0].curr_speed, ofport.properties[0].max_speed)
                # 无线端口时，无线网卡不给出最大带宽
                ...
                logger.info(" %d        %d      %s      %s      %.3f    %.3f", dpid, ofport.port_no, config, state, 
    ...
    def calculate_remain_bw(self, dpid, port_no, stats):
        ...
        if port_state:
            ...
            logger.info("  %d      %d  %d %d %d    %.2f     %d  %d  %d %d", 
    ...
    def _flow_stats_reply_handler(self, ev):
        ...
        logger.info("dpid  table_id   priority   eth_type   in_port   mpls_label    src_ip         dst_ip         out_port    count")
        # 获取交换机每个流表的信息
        for each_state in body:

            # logger.info("<network_monitor.py> --- each_state:", each_state)
            ...
            if setting.SHOW_FLOW_ENTRY:
                logger.info(" %2d      %2d       %5d      %s   %s    %s   %s     %s     %s    %s", 
