...
class NetworkOverloadNode(app_manager.RyuApp):
    ...
    def calculate_network_status(self, Graph):
        ...
        if self.monitor.port_remained_bw == {} or self.monitor.link_loss == {} or self.delay.link_delay == {} \
            ...
            logger.info("<overload_node.py> ---> Graph has not yet been enastablished")
            return None
    ...
    def find_overload_node(self):
        ...
        while True:
            ...
            logger.info("<overload_node.py> ---> len(network_status) is:", len(network_status))
            logger.info("<overload_node.py> ---> len(self.graph.nodes) is:", len(self.graph.nodes))
            if len(network_status) == len(self.graph.nodes):
                # 构建属性矩阵
                attribute_matrix = np.zeros((len(self.graph.nodes), 3))
                logger.info("<overload_node.py>  节点列表：{}".format(self.graph.nodes))
                ...
                logger.info("<overload_node.py> ---> the result of MADA is:", decision_scores)
                # 找到过载节点（得分最小的节点）
                overloaded_node_index = np.argmin(decision_scores)
                self.overloaded_node = list(self.graph.nodes)[overloaded_node_index]
                logger.info("<overload_node.py> ---> overloaded node is:", self.overloaded_node)
