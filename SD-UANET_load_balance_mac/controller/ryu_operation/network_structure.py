...
LastEditTime: 2024-11-27 10:50:34
...
class Networkstructure(app_manager.RyuApp):
    ...
    def _state_change_handler(self, ev):
        ...
        # 联机中断
            ...
            #     del self.sw_datapaths_table[datapath.id]
                # logger.info("<network_structure.py> --->  unreigster datapath: %016x" % datapath.id)
                # logger.info("<network_structure.py> --->  unreigster datapath: %016x" % datapath.id)
        # 交换机状态正常，允许所有操作交换机的行为
        self.sw_change_flag = False
    ...
    def get_topology(self, ev, way=0):
        ...
        if switch_list == [] or link_list == {}:
            logger.info("<network_structure.py> --->   switch_list or link_list is empty")
            return
        ...
        for each_switch in switch_list:
            ...
            # logger.info("<network_structure.py> --->   each_switch: ", each_switch)
        ...
        #     logger.info("<network_structure.py> --->  link_table reset")
        #     self.link_table = {}
        #     self.get_topology_num = 0
        ...
        for link in link_list:
            # logger.info("<network_structure.py> --->   link: ", link)
        ...
        if len(self.link_table) != 0:
            logger.info("<network_structure.py> ---> the link_table to assignment is:", self.link_table)
            self.link_table_backup = self.link_table
        else:
            logger.info("<network_structure.py> ---> link_table(zero) is:", self.link_table)
            logger.info("<network_structure.py> ---> link_table_backup is:", self.link_table_backup)
        ...
        #                 logger.info("the number of agree to delete dpid:{}".format(self.delete_sw_info[each_dpid]['delete_counter']))
        #                 logger.info("<structure.py> ---> topology was changed,remode {}'s information".format(each_dpid))
        ...
        logger.info("<structure.py>---not_use_ports:{}".format(self.not_use_ports)) # 交换机没有使用的端口

        logger.info("<structure.py>---sw_dpid_list:{}".format(self.sw_dpid_list))
    ...
    def build_topology_between_switches(self, bw=0.0, delay=0.0, loss=0.0, synthetic=100.0):
        ...
        # 新增：清空之前存储的链路数据包信息
        self.link_packet_storage.clear()
    ...
    def _packet_in_handler(self, ev):
        ...
        if eth_type == ether_types.ETH_TYPE_LLDP:
            # logger.info("<shortest_path.py>--->packet-in:LLDP Packet is detected, return") # 忽略LLDP报文
            return
        ...
        if isinstance(arp_pkt, arp.arp): 
            ...
            logger.info("<structure.py>--->arp_src_ip_src_ip:{}".format(arp_src_ip))
