...
class Host_Get_MSG(app_manager.RyuApp):
    def __init__(self, *_args, **_kwargs):
        ...
        self.search_link_request_method = re.compile(r'.+ClientState\)')
    ...
    def packet_in_handler(self, ev):
        ...
        if self.search_server_stats_method.search(data_str) != None:  
            # logger.info("<host_get_msg.py>   获取服务器的状态信息")
            ...
            logger.info("<host_get_msg.py>  the samba server {}'s status info：{}".format(src_host_ip, self.all_host_stats[src_host_ip]))

        # 2.检测是否是OVS交换机的状态信息
        elif self.search_switch_stats_method.search(data_str) != None:
            # logger.info("<host_get_msg.py>   获取OVS交换机的状态信息")
            ...
            logger.info("<host_get_msg.py>  the OVS switch {}'s statue info：{}".format(host_num, self.all_switch_stats[host_num]))

            # 新增：发送状态信息到客户端
            self.send_switch_stats_to_client(src_host_ip, Cpu_Uti, Mem_uti)

        # 3.检测是否是Client的存储文件请求
        elif self.search_request_method.search(data_str) != None: 
            if dst_cont_ip in setting.CONTROLLER_IP:
                
                logger.info("<host_get_msg.py>  request to transfer files!")
                ...
                if datapath == None:
                    logger.info("<host_get_msg.py> the datapath doest't exist")
                    return
                logger.info("\n★ ★ ★ ★ ★")
                logger.info("<host_get_msg.py>  receive a transfer request from the client!")
                ...
                if split_result:
                    ...
                    logger.info("<host_get_msg.py>   the split result is returned to the client %s" % src_host_ip)
                    logger.info("★ ★ ★ ★ ★ ★ \n")


        # 4. 新增：处理客户端请求链路信息的报文
        elif self.search_link_request_method.search(data_str) != None: 
            if dst_cont_ip in setting.CONTROLLER_IP:
                
                logger.info("<host_get_msg.py>  request to transfer Llink_info!")

                if self.structure is None:
                    self.structure = lookup_service_brick("structure")
                # 取出拓扑信息
                network_topology  = self.structure.network_topology  
                if network_topology is None:
                    logger.info("<host_get_msg.py> 未找到网络拓扑数据")
                    return

                # 序列化拓扑数据，采用 networkx 的 node_link_data 格式转换为 JSON 字符串
                topology_data = nx.node_link_data(network_topology)
                combined_payload = {
                    "switch_stats": self.all_switch_stats,
                    "topology": topology_data
                }
                topology_json = json.dumps(combined_payload)
                ...
                pkt.serialize()
                # 将序列化的拓扑 JSON 数据附加到数据包后面
                extra_data = topology_json.encode()  # 转换为字节流
                data = pkt.data + extra_data

                # 构造 OFPPacketOut 消息，将数据包发送给客户端
                actions = [parser.OFPActionOutput(port)]
                ...
                time.sleep(3)
                datapath.send_msg(out)
                logger.info("<host_get_msg.py> 已将网络拓扑数据发送给客户端 %s" % src_host_ip)
