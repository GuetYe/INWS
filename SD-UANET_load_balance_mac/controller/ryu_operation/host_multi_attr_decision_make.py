...
LastEditTime: 2024-11-25 20:31:46
LastEditors: Sun Shiquan
Description: 根据服务器的状态进行多属性决策获得文件的分割方案
FilePath: \SD-UANET_load_balance_2\controller\ryu_operation\host_multi_attr_decision_make.py
'''
...
class MutiAttribute_DecisionMaking(app_manager.RyuApp):
    ...
    def calculate(self, host_ip, file_name, file_size):
        ...
        if not self.host_get_msg.all_host_stats:
            logger.info("<host_MADM.py>      the samba server's status is empty MADM cannot be made！")
            return
        ...
        #         logger.info("<host_MADM.py>      主机 %s 没有状态上报，请检查" % each_host_ip)
        #         # 原本return
        #         # return
        ...
        logger.info("<host_MADM.py>   the list of samba servers is:%s" % self.host_get_msg.all_host_stats.keys())
        ...
        if np.sum(proportion_list) != 1.0:
            logger.info("<host_MADM.py>  The sum score of each client's split result is not 1, pleace check.")
        ...
        if sum != self.file_size:
            logger.info("<host_MADM.py>  File size check error, pleace check!")
            logger.info('<host_MADM.py>   The sum size of MADM method:', sum, type(sum))
            logger.info('<host_MADM.py>   The sum size of original file:', self.file_size, type(self.file_size))
            return None
        else:
            logger.info("<host_MADM.py>      The sum size of original file:%s" % self.file_size)
            logger.info("<host_MADM.py>      The sum size of MADM method:%s" % split_result)
            return split_result
