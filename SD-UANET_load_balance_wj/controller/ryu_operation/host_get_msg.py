...
class Host_Get_MSG(app_manager.RyuApp):
    def __init__(self, *_args, **_kwargs):
        ...
        self.search_link_request_method = re.compile(r'^\[ClientLinkRequest\(source=(?P<source>\w+)\)\]$')
    ...
    def packet_in_handler(self, ev):
        ...
        # 3.检测是否是Client的存储文件请求
            ...
            if dst_cont_ip in setting.CONTROLLER_IP:
                ...
                if self.MADM is None:
                    logger.error("MADM module is None! Possible state modification by another request.")
        ...
        # 4. 新增：处理客户端请求链路信息的报文
            ...
            if dst_cont_ip in setting.CONTROLLER_IP:
                ...
                logger.info("<host_get_msg.py>  request to transfer Link_info!")
                ...
                if combined_payload:
                    ...
                    pkt.serialize()
                    my_data = '[TypoResult(typo_result=%s)]' % combined_payload
                    data = pkt.data + bytearray(my_data.encode())
