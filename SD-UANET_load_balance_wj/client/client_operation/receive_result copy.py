...
class ReceivePacket:
    ...
    def parse_control_packet(self, arp_packet_load):
        ...
        # 检查是否为链路信息数据包
        if "switch_stats" in self.typo_dict or "topology" in self.typo_dict:
            print("<receive_result> --> 数据包中包含拓扑信息和交换机状态信息")
            self.parse_and_store_packet(self.typo_dict)

        elif '"SplitResult"' in arp_packet_load:
            print("<receive_result> --> File split result packet detected.")
            result = re.findall(pattern='\{.+\}', string=arp_packet_load, )
            if result:
                self.receive_dict = json.loads(result[0])
                print("<receive_result> Split result:")
                print(json.dumps(self.receive_dict, indent=4, ensure_ascii=False))
            else:
